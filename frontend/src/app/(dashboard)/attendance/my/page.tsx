...
function StatusBadge({ record }: { record: AttendanceResponse }) {
  ...
  return <span className={`inline-flex shrink-0 whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${statusStyle[status]}`}>{status}</span>;
}
...
export default function MyAttendancePage() {
  ...
  return <div className="mx-auto min-w-0 max-w-[1600px] space-y-4 overflow-x-clip sm:space-y-5">
    <section className="flex min-w-0 flex-col gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-5">
      ...
      <div className="grid w-full min-w-0 grid-cols-[40px_minmax(0,1fr)_40px] gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
        ...
        <input type="month" value={month} onChange={(event) => setMonth(event.target.value)} className="h-10 min-w-0 w-full rounded-md border border-gray-200 px-3 text-sm font-medium text-gray-700 outline-none focus:border-blue-500 sm:w-auto" />
        ...
        <button type="button" onClick={() => setMonth(currentMonth())} className="col-span-3 h-10 w-full whitespace-nowrap rounded-md border border-gray-200 px-3 text-sm font-medium text-gray-600 hover:bg-gray-50 sm:w-auto">이번 달</button>
      </div>
    </section>

    <section className="grid gap-3 sm:grid-cols-3 sm:gap-4">{cards.map((card) => { const Icon = card.icon; return <div key={card.label} className="flex min-w-0 items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:gap-4 sm:p-5"><span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${card.style}`}><Icon className="h-5 w-5" /></span><div className="min-w-0"><p className="text-sm font-medium text-gray-500">{card.label}</p><p className="mt-1 break-keep text-lg font-bold text-gray-900 sm:text-xl">{card.value}</p></div></div>; })}</section>

    <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5"><h2 className="font-bold text-gray-900">근태 상태 요약</h2><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">{[["정상 출근", summary.normal, "text-emerald-600"], ["지각", summary.late, "text-orange-600"], ["조퇴", summary.early, "text-violet-600"], ["결근", summary.absent, "text-rose-600"]].map(([label, value, color]) => <div key={String(label)} className="min-w-0 rounded-lg bg-gray-50 px-3 py-3 sm:px-4"><p className="break-keep text-xs font-medium text-gray-500">{label}</p><p className={`mt-1 text-lg font-bold sm:text-xl ${color}`}>{value}회</p></div>)}</div></section>

    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-gray-200 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5"><h2 className="font-bold text-gray-900">일별 근태 목록</h2><div className="flex flex-nowrap gap-2 overflow-x-auto sm:flex-wrap">{filters.map((item) => <button key={item.value} type="button" onClick={() => setFilter(item.value)} className={`shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-semibold ${filter === item.value ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{item.label}</button>)}</div></div>
      {loading ? <div className="py-16 text-center text-sm text-gray-500">근태 기록을 불러오는 중입니다.</div>
        ...
        <div className="divide-y divide-gray-100 md:hidden">{shown.map((item) => <button key={item.attendanceId} type="button" onClick={() => setDetail(item)} className="w-full p-4 text-left hover:bg-gray-50"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-semibold">{formatDate(item.workDate)}</p><p className="text-xs text-gray-500">{weekday(item.workDate)}</p></div><StatusBadge record={item} /></div><div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 text-sm text-gray-600"><span className="whitespace-nowrap">출근 {formatTime(item.checkInTime)}</span><span className="whitespace-nowrap">퇴근 {formatTime(item.checkOutTime)}</span><span className="col-span-2 break-keep">근무 {formatMinutes(item.workMinutes)}</span></div></button>)}</div></>}
    </section>
    ...
    {detail && <Modal
      ...
      footer={<>
        {needsCorrection(detail.attendanceStatusCode) && <button type="button" onClick={() => setCorrecting(true)} className="flex w-full items-center justify-center gap-1.5 whitespace-nowrap rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-600 hover:bg-blue-100 sm:w-auto"><PencilSquareIcon className="h-4 w-4" />{detail.reason ? "사유 수정" : "정정"}</button>}
        <ModalCancelButton onClick={() => setDetail(null)}>닫기</ModalCancelButton>
      </>}
    >
      <dl className="divide-y divide-gray-100">{[["날짜", formatDate(detail.workDate)], ["출근 시간", formatTime(detail.checkInTime)], ["퇴근 시간", formatTime(detail.checkOutTime)], ["근무 시간", formatMinutes(detail.workMinutes)]].map(([label, value]) => <div key={label} className="flex justify-between gap-4 py-3 first:pt-0"><dt className="shrink-0 text-sm text-gray-500">{label}</dt><dd className="min-w-0 break-words text-right text-sm font-semibold">{value}</dd></div>)}<div className="flex items-center justify-between gap-4 py-3"><dt className="text-sm text-gray-500">근태 상태</dt><dd><StatusBadge record={detail} /></dd></div>
      {needsCorrection(detail.attendanceStatusCode) && <div className="py-3"><dt className="text-sm text-gray-500">사유</dt><dd className="mt-1 break-words text-sm font-medium text-gray-800">{detail.reason || "등록된 사유가 없습니다."}</dd>{detail.attachmentName && <dd className="mt-1 break-words text-xs">{detail.attachmentUrl ? <a href={`${API_BASE_URL.replace(/\/api\/?$/, "")}${detail.attachmentUrl}`} target="_blank" rel="noreferrer" className="break-words text-blue-600 hover:underline">{detail.attachmentName}</a> : detail.attachmentName}</dd>}</div>}
      </dl>
    </Modal>}
