...
export default function LeaveApprovalStats({ summary, loading }: { summary: LeaveRequestSummary | null; loading: boolean }) {
  ...
  return <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:gap-4">{cards.map((card) => { const Icon = card.icon; return <div key={card.label} className="flex min-w-0 items-center justify-between rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5"><div className="min-w-0"><p className={`text-sm font-medium ${card.color}`}>{card.label}</p><p className="mt-2 whitespace-nowrap text-2xl font-bold text-gray-900 sm:text-3xl">{loading ? "-" : `${card.value}건`}</p></div><div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ${card.bg} ${card.color}`}><Icon className="h-6 w-6" /></div></div>; })}</div>;
}
