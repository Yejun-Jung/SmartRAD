...
export default function MonthlyAttendanceDashboard() {
  ...
  return <div className="mx-auto min-w-0 max-w-[1600px] space-y-4 overflow-x-clip pb-8">{error && <div className="flex flex-col gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 sm:flex-row sm:items-center sm:justify-between"><span className="break-words">{error}</span><button type="button" onClick={() => setRetryKey((key) => key + 1)} className="w-full rounded-md border border-rose-200 bg-white px-3 py-1.5 font-semibold hover:bg-rose-50 sm:w-auto">재시도</button></div>}<MonthlyAttendanceStats data={data} loading={loading} /><div className="grid min-w-0 grid-cols-1 items-start gap-4 lg:grid-cols-[minmax(0,1fr)_260px] xl:grid-cols-[minmax(0,1fr)_280px]"><DailyAttendanceTrend month={selectedMonth} data={data?.trend ?? []} loading={loading} /><MonthlyAttendanceDonut data={data} loading={loading} /></div><div className="grid min-w-0 grid-cols-1 items-start gap-4 lg:grid-cols-2 xl:grid-cols-[220px_minmax(0,1fr)_260px]"><DepartmentAttendanceRates data={data?.departments ?? []} /><EmployeeMonthlySummary rows={data?.employees ?? []} /><AttendanceWarningEmployees rows={data?.employees ?? []} /></div></div>;
}
