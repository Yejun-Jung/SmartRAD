  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const allEmployees = employeePage.content ?? [];
        if (!cancelled) setData(aggregateMonthly(selectedMonth, allEmployees, Array.isArray(summaries) ? summaries : [], byDate, departmentsList));
